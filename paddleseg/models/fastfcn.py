...
class FastFCN(nn.Layer):
    def __init__(self,
        ...
        if self.add_lateral:
            self.lateral_convs = nn.LayerList(
                [
                    layers.ConvBNReLU(in_channels[0], mid_channels, 1, bias_attr=False),
                    layers.ConvBNReLU(in_channels[1], mid_channels, 1, bias_attr=False),
                ]
            )
        ...
        if self.aux_loss:
            self.fcn_head = layers.AuxLayer(in_channels[-2], mid_channels, num_classes)
    ...
    def forward(self, inputs):
        ...
        out = F.interpolate(out, size=imsize, mode='bilinear', align_corners=False)
        output = [out]

        if self.training:
            fcn_out = self.fcn_head(fcn_feat)
            fcn_out = F.interpolate(fcn_out, size=imsize, mode='bilinear', align_corners=False)
...
class Encoding(nn.Layer):
    ...
    def aggregate(assignment_weights, x, codewords):
        ...
        expanded_x = paddle.tile(x.unsqueeze(2),
            [1, 1, num_codes, 1],
        )
